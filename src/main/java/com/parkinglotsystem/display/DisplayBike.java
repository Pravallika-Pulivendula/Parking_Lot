...
public class DisplayBike implements Display {
    ...
    public void displayFreeCount(ArrayList<ParkingFloor> floors) {
        ...
        while (floor < floors.size()) {
            System.out.println("No of free slots for BIKE on floor " + (floor + 1) + ": " + this.display(floors).get(floor).get(0).size());
            floor++;
        }
    }
    ...
    public void displayFreeSlots(ArrayList<ParkingFloor> floors) {
        int floor = 0;
        while (floor < floors.size()) {
            System.out.println("Free slots for BIKE on floor " + (floor + 1) + ": " + this.display(floors).get(floor).get(0).toString().replace("[", "").replace("]", ""));
            floor++;
        }
    }
    ...
    public void displayOccupiedSlots(ArrayList<ParkingFloor> floors) {
        int floor = 0;
        while (floor < floors.size()) {
            System.out.println("Occupied slots for BIKE on floor " + (floor + 1) + ": " + this.display(floors).get(floor).get(1).toString().replace("[", "").replace("]", ""));
            floor++;
        }
    }
}
